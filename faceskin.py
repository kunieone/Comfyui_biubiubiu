...
FACESKIN_CKPT = os.path.join(folder_paths.models_dir, "biubiubiu/face_skin.pth")
BBREGRESSOR_PARAM = os.path.join(folder_paths.models_dir, "biubiubiu/BBRegressorParam_r.mat")


def color_transfer(sc, dc, mask):
    """
    ...
    def get_mean_and_std(img):
        x_mean, x_std = cv2.meanStdDev(img*mask)
...
class Face_Skin(object):
    ...
    # 17:'hair', 18:'hat'
    def __call__(self, image, bbox, needs_index=[[12, 13]]):
        ...
        with torch.no_grad():
            ...
            for _needs_index in needs_index:
                ...
                masks.append(Image.fromarray(np.uint8(sub_mask)))
...
class FaceSkinSegmentation:
    def __init__(self):
        self.face_skin_model = Face_Skin(FACESKIN_CKPT)
    ...
    RETURN_TYPES = ("IMAGE", "MASK", "BBOXDETAIL")
    RETURN_NAMES = ("image_croped", "mask_croped", "bbox detail")
    ...
    def run_it(self, image, insightface, resized, expand_ratio):
        ...
        for img in image:
            ...
            skin_mask = self.face_skin_model(img, bbox,
                                        needs_index=[[1, 2, 3, 4, 5, 10, 11, 12, 13]])[0]
            
            skin_mask = pil2tensor(skin_mask.resize((resized, resized)) if resized != -1 else img_crop)[..., 0:1]
            image_crop_list.append(img_crop)
            image_mask_list.append(skin_mask)
            bbox_list.append(bbox)
        mask = torch.concat(image_mask_list, dim=0).permute(0, 3,1,2)
        croped = torch.concat(image_crop_list, dim=0)

        return (croped, mask, bbox_list)
...
class FaceSkinPaste:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "dilate_size": ("INT", {"default": 64}),
                "blur_size": ("INT", {"default": 32})
    ...
    def run_it(self, image, image_croped, mask, bbox_detail, dilate_size, blur_size):
        bs = image.shape[0]
        tensors = []
        for i in range(bs):
            ...
            mask_croped = mask[i].cpu().numpy()[..., None]
            bbox = bbox_detail[i]

            raw_croped = cv2.resize(img[bbox[1]:bbox[3], bbox[0]:bbox[2]], (img_croped.shape[1], img_croped.shape[0]))
            kernel = np.ones((dilate_size,dilate_size),np.uint8)
            # face_skin_mask_dilate = cv2.dilate(mask_croped.astype(np.uint8), kernel)[..., None]

            shift_image = color_transfer(np.uint8(img_croped*255), 
                                         np.uint8(raw_croped*255), mask_croped).astype(np.float32)/255

            face_skin_mask = cv2.blur(mask_croped, (blur_size, blur_size))[..., None]

            paste = shift_image * face_skin_mask + np.array(img_croped) * (1 - face_skin_mask)
            paste = cv2.resize(paste, (bbox[3]-bbox[1], bbox[2]-bbox[0]))
